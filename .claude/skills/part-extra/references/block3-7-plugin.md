...
Plugin이 무엇인지 **"둘러보기"** 수준으로 체험한다. 본 3시간 강의에서는 별도 설치를 요구하지 않는다 — 핵심 스킬(clarify 계열, wrap-session 등)은 이미 이 레포(`heum-3h/`)에 직접 포함되어 있기 때문이다.
...
```
heum-3h/.claude/
├── skills/          ← 여러 skill이 담긴 폴더 (clarify 계열, wrap-session 등)
├── agents/          ← wrap-session이 호출하는 전문 에이전트들
