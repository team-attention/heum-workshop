# Block 3: wrap-history로 세션 히스토리 분석
...
Block 2에서 wrap-session 스킬을 실행했습니다. 그 실행 기록이 어딘가에 저장되어 있습니다. 이번 블록에서는 그 "저장된 기록"을 들여다보는 방법을 배웁니다. wrap-session이 만들어낸 결과뿐 아니라, 과거 모든 세션의 기록에서 패턴을 찾아주는 도구가 바로 **wrap-history**입니다.
...
**비유하면:**
일기장을 매일 쓴다고 생각해보세요. 한 달 뒤에 일기장을 다시 펼쳐보면 "아, 이번 달엔 이런 고민을 많이 했구나", "이런 실수를 자주 했네"라는 패턴이 보입니다. Claude Code도 마찬가지입니다. 쌓여 있는 대화 기록에서 패턴을 찾아주는 것이 바로 **wrap-history** 스킬입니다.

### wrap-history 스킬은 뭘 하나?
...
세션 파일에는 실제 대화 외에도 파일 변경 이력, 큐 작업 같은 **부속 자료**가 대량으로 포함되어 있습니다. wrap-history는 불필요한 부분을 걸러내고 핵심만 추출합니다.
...
비유하자면, 두꺼운 일기장에서 실제로 쓴 글은 아주 일부분이고, 나머지는 날짜 도장, 날씨 기록, 영수증 같은 부속 자료인 셈입니다. **wrap-history는 이 부속 자료를 다 걷어내고, 실제 대화만 모아서 분석합니다.** 그래서 아무리 기록이 많아도 빠르게 핵심만 뽑아낼 수 있습니다.

---

### 활용 사례

| 질문 | wrap-history가 해주는 것 |
|------|---------------------------|
...
wrap-history 스킬이 설치되어 있는지 확인합니다. Claude Code에서:

```
어떤 스킬이 설치되어 있어?
```

목록에 `wrap-history`가 있으면 아래 단계로 진행합니다. 없다면 강사에게 문의하세요.
...
wrap-history 원본 스킬에는 대량의 세션 파일을 처리하는 고급 기능(배치 파이프라인, 병렬 분석)이 있습니다. 궁금하면 Claude에게 물어보세요:

```
wrap-history 플러그인의 원본 SKILL.md에서 Batch Extract Pipeline 부분을 설명해줘
```
...
정답: 1번. 세션 파일의 대부분은 파일 변경 이력 같은 부속 자료다. 실제 사용자와 Claude의 대화는 약 6%만 차지한다. wrap-history는 이 핵심 대화만 추출해서 분석하므로, 큰 파일도 빠르게 처리된다.
